...
impl ContentSource {
    ...
    pub fn new() -> Self {
        ...
        let timestamp = uuid::Timestamp::from_unix(
            uuid::NoContext,
            now.timestamp().as_second() as u64,
            now.timestamp().subsec_nanosecond() as u32,
        );
        Self {
...
mod tests {
    ...
    fn test_from_uuid() {
        ...
        let source = ContentSource::from_uuid(uuid);
        assert_eq!(source.as_uuid(), uuid);
    }
    ...
    fn test_conversions() {
        ...
        let uuid = Uuid::new_v7(timestamp);
        let source: ContentSource = uuid.into();
        let back_to_uuid: Uuid = source.into();
