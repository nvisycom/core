...
pub trait AsyncContentRead: AsyncRead + Unpin + Send {
    ...
    ///
    /// ```no_run
    /// use nvisy_core::{io::{AsyncContentRead, ContentData}, ContentSource};
    /// use tokio::fs::File;
    /// use std::io;
    ///
            ...
            loop {
                ...
                if total_read + bytes_read > max_size {
                    return Err(io::Error::new(
                        ...
                        format!("Content size exceeds maximum limit of {} bytes", max_size),
                    ));
                }
...
mod tests {
    ...
    async fn test_read_content_verified() -> Result<()> {
        ...
        let content = cursor.read_content_verified(|data| data.len() > 0).await?;
        assert_eq!(content.as_bytes(), data);
    ...
    async fn test_read_content_verified_fails() -> Result<()> {
        ...
        let result = cursor.read_content_verified(|data| data.is_empty()).await;
        assert!(result.is_err());
