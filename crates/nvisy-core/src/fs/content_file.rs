...
use crate::fs::{ContentKind, ContentMetadata, SupportedFormat};
...
impl ContentFile {
        ...
        loop {
            ...
            if total_read + bytes_read > max_size {
                return Err(Error::new(
                    ErrorType::Runtime,
                    ErrorResource::Core,
                    format!("File size exceeds maximum limit of {} bytes", max_size),
                ));
            }
    ...
    /// Detect supported format from file extension
    ///
    /// # Example
    ///
    /// ```no_run
    /// use nvisy_core::fs::{ContentFile, SupportedFormat};
    ///
    /// async fn example() -> Result<(), Box<dyn std::error::Error>> {
    ///     let content_file = ContentFile::open("document.pdf").await?;
    ///     if let Some(format) = content_file.detect_supported_format() {
    ///         println!("Format: {} ({})", format, format.description());
    ///         println!("Type: {}", format.format_type());
    ///     }
    ///     Ok(())
    /// }
    /// ```
    pub fn detect_supported_format(&self) -> Option<SupportedFormat> {
        self.extension().and_then(SupportedFormat::from_extension)
    }
...
mod tests {
    ...
    async fn test_file_operations() {
        ...
        // Test existence
        assert!(content_file.exists().await);
